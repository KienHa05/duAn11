...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-4xl font-bold">Chỉnh sửa sản phẩm</h1>
            <p class="text-base-content/60 mt-1">{{ $product->name }}</p>
        </div>
    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-control">
                            ...
                            <textarea name="description" placeholder="Mô tả chi tiết sản phẩm..." rows="4" 
                                class="textarea textarea-bordered focus:textarea-primary text-base @error('description') textarea-error @enderror">{{ old('description', $product->description) }}</textarea>
                            @error('description')
        ...
        <div class="space-y-6">
            ...
            <div class="card bg-base-100 shadow-md">
                <div class="card-body">
                    ...
                    @if($product->image)
                        <div class="mb-4 rounded-xl overflow-hidden border-2 border-base-200">
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" 
                                class="w-full h-auto max-h-64 object-cover hover:scale-105 transition duration-300">
                            <div class="bg-base-200 p-2 text-center text-xs text-base-600">
                                📁 Ảnh hiện tại ({{ number_format(random_int(50, 500)) }} KB)
                            </div>
                        </div>
                    @endif

                    <!-- Upload Area -->
                    <div id="imagePreview" class="w-full aspect-square bg-gradient-to-br from-base-200 to-base-300 rounded-xl border-2 border-dashed border-base-400 flex items-center justify-center mb-4 overflow-hidden cursor-pointer transition hover:border-primary hover:from-primary/5 hover:to-primary/10 group {{ $product->image ? 'hidden' : '' }}">
                    ...
                    <div class="flex gap-2 mb-3">
                        <button type="button" onclick="document.getElementById('imageInput').click()" class="btn btn-primary flex-1 gap-2">
                            <x-heroicon-o-arrow-up-tray class="w-5 h-5" /> {{ $product->image ? 'Đổi ảnh' : 'Chọn ảnh' }}
                        </button>
                        <button type="button" id="clearImageBtn" class="btn btn-ghost btn-square {{ $product->image ? '' : 'hidden' }}">
            ...
            <div class="flex flex-col gap-3">
                <button type="submit" form="productForm" class="btn btn-primary btn-lg gap-2">
                    <x-heroicon-o-check-circle class="w-6 h-6" /> Lưu thay đổi
                </button>
            ...
            <div class="alert alert-error alert-sm">
                ...
                <div>
                    <h3 class="font-bold text-sm">Vùng nguy hiểm</h3>
                    <p class="text-xs mt-1">Xóa sản phẩm vĩnh viễn (không thể khôi phục)</p>
                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="mt-2" onsubmit="return confirm('⚠️ Xác nhận xóa? Hành động này không thể hoàn tác!')">
                        @csrf
                        @method('DELETE')
...
function handleImageSelect(e) {
    ...
    reader.onload = (e) => {
        ...
        selectBtn.querySelector('x-heroicon-o-arrow-up-tray')?.remove() || true;
        selectBtn.innerHTML = '✎ Đổi ảnh';
...
function clearImage() {
    ...
    selectBtn.innerHTML = '<svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg> Chọn ảnh';
